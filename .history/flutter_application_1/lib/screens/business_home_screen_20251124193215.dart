...
class _BusinessHomeScreenState extends State<BusinessHomeScreen> {
  ...
  Future<void> _toggleServiceStatus(Service service, bool isActive) async {
    ...
    try {
      final updated =
          await _businessService.updateService(service.id!, isActive: isActive);
      if (!mounted) return;
  ...
  Future<void> _deleteEmployee(Employee employee) async {
    ...
    setState(() {
      _serviceActionLoading.add(employee.id!);
    });
    ...
    try {
      ...
      if (mounted && employee.id != null) {
        setState(() {
          _serviceActionLoading.remove(employee.id!);
        });
      }
    }
  }
  ...
  Future<void> _showEmployeeForm({Employee? employee}) async {
    ...
    final specializationController =
        TextEditingController(text: employee?.specialization ?? '');
  ...
  Future<void> _showWorkingHoursSheet() async {
    ...
    if (result != null && result['opening'] != null && result['closing'] != null) {
  ...
  Widget _buildServiceCard(Service service) {
    final isBusy =
        service.id != null && _serviceActionLoading.contains(service.id!);
    final cardColor = service.isActive ? Colors.white : Colors.grey.shade50;
        ...
        children: [
          Row(
            children: [
              ...
              Switch.adaptive(
                ...
                onChanged: (service.id == null || isBusy)
                    ? null
                    : (value) => _toggleServiceStatus(service, value),
          ...
          Row(
            children: [
              TextButton.icon(
                onPressed:
                    isBusy ? null : () => _showServiceForm(service: service),
                icon: const Icon(Icons.edit, size: 18),
                label: const Text('Düzenle'),
              ),
              const SizedBox(width: 8),
  ...
  Widget _buildEmployeeCard(Employee employee) {
    final isBusy =
        employee.id != null && _serviceActionLoading.contains(employee.id!);
        ...
        children: [
          Row(
            children: [
              ...
              Switch.adaptive(
                ...
                onChanged: (employee.id == null || isBusy)
                    ? null
                    : (value) => _toggleEmployeeStatus(employee, value),
                activeColor: Colors.green,
              ),
            ],
          ),
          ...
          Row(
            children: [
              TextButton.icon(
                onPressed:
                    isBusy ? null : () => _showEmployeeForm(employee: employee),
                icon: const Icon(Icons.edit, size: 18),
                label: const Text('Düzenle'),
              ),
              const SizedBox(width: 8),
